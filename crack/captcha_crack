...
if ($utilisateur['grade'] == 1) {
    if (!empty($_SESSION['token_gen'])) {
        if ($_SESSION['token_gen'] < time()-10 AND $_SESSION['token_gen'] > time()-180) {
            if (!isset($_POST['generer']) AND strpos($_SERVER['HTTP_REFERER'], 'cu6.io') != (7 OR 8)) {
                header('Location: /');
                exit();
            }
        } else {
    ...
    if (!empty($_SESSION['token_gen'])) {
        if ($_SESSION['token_gen'] > time()-180) {
            if (strpos($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) != (7 OR 8)) {
                unset($_SESSION['token_gen']);
                header('Location: /');
                exit();
            }
                ...
                if (check_statut_gen($stock, $utilisateur['grade'], $id)) {
                    ...
                    if($utilisateur['grade'] == 1){
                        if ($utilisateur['generations_jour'] <= $limite_nvip['valeur']) {
                            ...
                            unset($_SESSION['token_gen']);
                            header('Location: /generation/'.$generateur['id'].'/'.$compte['compte']);
                            exit();
                        } else {
                            $erreur['type'] = 'danger';
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="https://freegen.pw">FreeGen</a></li>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <?php } ?>
                    <h4 class="header-title mt-0 mb-3">Sécurité captcha.</h4>
                    <p class="text-muted font-13" id="message">Passez le pour avoir votre compte</p>
                    <hr>
                    <form method="POST">
                        ...
                        <hr>
                        <div class="form-group">
                            <input type="submit" name="generer" class="btn btn-primary" value="Générer"/>
                        </div>
